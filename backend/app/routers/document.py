...
async def search_documents(
    ...
    try:
        ...
        if document_ids:
            try:
                selected_document_ids = json.loads(document_ids)
            except json.JSONDecodeError:
                selected_document_ids = [document_ids]
            docs = db.query(DocumentDB).filter(DocumentDB.id.in_(selected_document_ids)).all()
            backend_doc_ids = [
                doc.document_id or f"{doc.id}_{doc.filename}"
                for doc in docs
            ]
    ...
    try:
        ...
        try:
            ...
            if document and result['success']:
                ...
                document.document_id = result.get('document_id', document.document_id)
                db.commit()
