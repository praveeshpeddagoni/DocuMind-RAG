...
def check_environment():
    ...
    # FAISS CPU is supported; GPU APIs are only present in faiss-gpu.
    try:
        import faiss
        if hasattr(faiss, "StandardGpuResources"):
            logger.info("FAISS GPU support available")
        else:
            logger.info("FAISS CPU support available")
    except ImportError:
        issues.append("Missing FAISS package (install faiss-cpu)")
